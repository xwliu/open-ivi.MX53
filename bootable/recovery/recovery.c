...
extern int mcu_update(void);
extern int mpeg_update(void);
extern int mcu_reset_cpu(void);
extern int system_update_end;
extern int system_cleanboot_end;
extern const char mcu_dev[];
static const char *RECOVERY_FILE = "/sdcard/tractor/update.zip";
...
#else
	static const char *TEMPORARY_LOG_FILE = "/dev/ttymxc0";
#endif
...
prepend_title(const char** headers) {
    char* title[] = {"recovery by TokenwireLess",
    		      NULL
                    };
...
{
	int fd = 0;
	fd= open( "/dev/mcu_i2c", O_RDWR | O_CREAT | O_TRUNC );
	if (fd < 0) {
		LOGE("open mcu dev error\n");
	} else {
		int val = 0;
		ioctl(fd, MCU_SET_BACKLIGHT, &val);
	}
}


void erase_firmware_app()
{
	...
	sprintf(buf_cmd, "/system/bin/busybox rm -rf /firmware/app");
	system(buf_cmd);
	printf("%s	rt=%d	\n\r",buf_cmd, rt_cmd);
}

void copy_app_2_data()
{
	DIR *dp;
	struct dirent *d;
	int factory_apk_sts = 0;
	int factory_db_sts = 0;
	if(mount("/dev/block/mmcblk0p2", "/data", "ext4",
		MS_NOATIME | MS_NODEV | MS_NODIRATIME, "") < 0) {
		printf("mount data failure\n");
	}
	if(mount("/dev/block/mmcblk0p6", "/firmware", "ext4",
		MS_NOATIME | MS_NODEV | MS_NODIRATIME, "") < 0) {
		printf("mount firmware failure\n");
	...
	/*find Factory.apk */
	if ((dp = opendir("/firmware/app")) == NULL) {
		printf("unable to open directory /firmware/app\n");
		return;
	}	
	while ((d = readdir(dp)) != NULL) {
		if(strcmp("Factory.apk", d->d_name) == 0) {
			printf("Factory.apk is found\n");
			factory_apk_sts = 1;
			break;
		}
	}
	if(factory_apk_sts == 1) {
		/*find factory.db*/
#ifdef RECOVERY_FACTORY_DISABLE
		factory_db_sts = 1;
#else
		if ((dp = opendir("/firmware/factory")) == NULL) {
			printf("unable to open directory /firmware/factory\n");
		}
		if(dp != NULL) {
			while ((d = readdir(dp)) != NULL) {
				if(strcmp("factory.db", d->d_name) == 0) {
					printf("factory.db is found\n");
					factory_db_sts = 1;
					break;
				}
			}
...
#endif
		if(factory_db_sts == 1) {
			/*remove factory.apk from /data/app*/
			sprintf(buf_cmd, "/system/bin/busybox rm -rf /data/app/Factory.apk");
			system(buf_cmd);
			printf("%s\n\r",buf_cmd);
			/*remove remove factory.apk from /firmware/app*/
			sprintf(buf_cmd, "/system/bin/busybox rm -rf /firmware/app/Factory.apk");
			system(buf_cmd);
			printf("%s\n\r",buf_cmd);
		} else {
			/*only keep factory.apk in /data/app*/
			sprintf(buf_cmd, "/system/bin/busybox rm -rf /data/app/*");
			system(buf_cmd);
			printf("%s\n\r",buf_cmd);

			sprintf(buf_cmd, "/system/bin/busybox mv /firmware/app/Factory.apk /data/app/");
			system(buf_cmd);
			printf("%s\n\r",buf_cmd);
		}
	} 
...
void remove_logo_config_file()
{
	...
	sprintf(buf_cmd, "/system/bin/busybox rm -rf /firmware/config");
	system(buf_cmd);
	printf("%s\n\r",buf_cmd);
}
...
{
	char *cmd_buf[256];
	//in clean boot mode, there is 2 steps
	//1. format /data partition
	//2. copy /firmware backup data to /data partitions
	erase_volume("/data");	 
	erase_volume("/cache");	 
	...
	system(buf_cmd);
	printf("%s\n\r",buf_cmd);
	copy_file("/firmware/", "/data/", 1 | 4 | 8 );
	sleep(3);
}
extern int install_recovery(const char *path);
int set_recovery(void) 
{
	int result = 0;
	FILE* fp_done = fopen("/sdcard/tractor/recovery","w");
	if(fp_done == NULL){
		LOGE("Error to disable auto burn\n");
		result = INSTALL_ERROR;
	}else{
		...
		fwrite(buffer, 1, 16, fp_done);
		fclose(fp_done);
		printf("Burning recovery done, and set the status to update system\n");
		result = INSTALL_SUCCESS;
	}
	/*remove skip file*/
	char *cmd_buf[256];
	sprintf(buf_cmd, "/system/bin/busybox rm -rf /sdcard/tractor/skip");
	system(buf_cmd);
