...
Value* DisplaySeekbarFn(const char* name, State* state, int argc, Expr* argv[]) {
    if (argc != 1) {
        return ErrorAbort(state, "%s() expects 1 arg, got %d", name, argc);
    }
    char* frac_str;
    ...
    int frac = atol(frac_str);

    UpdaterInfo* ui = (UpdaterInfo*)(state->cookie);
    fprintf(ui->cmd_pipe, "Display_seekbar %d\n", frac);
...
SimpleDDFn(const char *name, State *state, int argc, Expr * argv[])
{
	...
	char *infile_buf = malloc(strlen(inputfile)+1);
	strcpy(infile_buf, inputfile);	

	char *outfile_buf = malloc(strlen(outfile)+1);
	strcpy(outfile_buf, outfile);

//    char* infile_name = strtok(infile_buf, "\n");
//    char* outfile_name = strtok(outfile_buf, "\n");

	UpdaterInfo* ui = (UpdaterInfo*)(state->cookie);
    //fprintf(ui->cmd_pipe, "simple_dd %s %s %l\n", infile_buf, outfile_buf, seek_offset);	
    
	char *update_info = malloc(128);
	sprintf(update_info, "%s %s %d\n", infile_buf, outfile_buf, seek_offset);
    fprintf(ui->cmd_pipe, "simple_dd %s\n", update_info);
#endif
	return StringValue(strdup("t"));
error:
#if 0
...
void RegisterInstallFunctions() {
    ...
    RegisterFunction("display_seekbar", DisplaySeekbarFn);
