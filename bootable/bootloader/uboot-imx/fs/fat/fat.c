...
static int disk_read(__u32 startblock, __u32 getsize, __u8 *bufptr)
{
    ...
    if(startblock + getsize > cur_dev->lba){
        printf("Warning disk_read over boundary!");
        printf("Startblock: 0x%x, Size: 0x%x, Cur_lba: 0x%x\n", startblock, getsize, cur_dev->lba);
        return -1;
    }
...
int fat_register_device(block_dev_desc_t *dev_desc, int part_no)
{
	...
	if (!get_partition_info(dev_desc, part_no, &info)) {
		...
		cur_part = part_no;
		/* Debug log for reading SDcard in high-temperature environments*/
		/*Aibing debug, remove the debug msg for boot time optimize*/
        //printf("MMC dev: %d, Bnum: 0x%x, Bsize: 0x%x\n", dev_desc->dev, dev_desc->lba, dev_desc->blksz);
		//printf("Part Start: 0x%x, Bnum: 0x%x, Bsize: 0x%x\n", info.start, info.size, info.blksz);
