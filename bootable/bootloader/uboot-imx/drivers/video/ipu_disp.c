...
void ipu_dmfc_set_burst_size(int dma_chan, int burst_size)
{
	...
	case 16:
		...
		break;
	default:
		printf("Unsupported burst size %d\n", burst_size);
		return;
	}
...
void ipu_dp_dc_enable(ipu_channel_t channel)
{
	...
	uint32_t reg;
	uint32_t dc_chan;
	//printf("ipu_dp_dc_enable: 0x%x VS 0x%x\n", channel, MEM_FG_SYNC);
	if (channel == MEM_FG_SYNC)
		dc_chan = 5;
				  ...
				  uint8_t alpha)
{
	...
	//printf("curr bg_chan value: %d, enable: %d, value: %d", bg_chan, enable, alpha);
	if (bg_chan) {
