...
static int mxcfb_map_video_memory(struct fb_info *fbi, int is_overlay);
...
static int setup_disp_channel1(struct fb_info *fbi)
{
	...
	if(mxc_fbi->overlay)
		params.mem_dp_fg_sync.di = mxc_fbi->ipu_di;		
	else
	 ...
	 */
	 if(mxc_fbi->overlay){
	 	if (fbi->var.vmode & FB_VMODE_INTERLACED) {
			params.mem_dp_fg_sync.interlaced = 1;
		...
		params.mem_dp_fg_sync.in_pixel_fmt = bpp_to_pixfmt(fbi);
	 }else{
		if (fbi->var.vmode & FB_VMODE_INTERLACED) {
 ...
 * @param       info     framebuffer information pointer
 */
static int mxcfb_set_par(struct fb_info *fbi, int is_overlay)
{
	int retval = 0;
	u32 mem_len;
	...
	if (!fbi->fix.smem_start || (mem_len > fbi->fix.smem_len)) {
		...
		if (mxcfb_map_video_memory(fbi, is_overlay) < 0)
			return -ENOMEM;
	}
	...
	if (fbi->var.vmode & FB_VMODE_INTERLACED) {
		...
		if (mxc_fbi->ipu_di_pix_fmt){
			...
			//printf("mxcfb_set_par out_pixel_fmt: 0x%x\n", out_pixel_fmt);
		}
		else
...
static int mxcfb_map_video_memory(struct fb_info *fbi, int is_overlay)
{
	...
	if(is_overlay){
		fbi->screen_base = (char*)0x97D00000;
		fbi->fix.smem_start = (unsigned long)0x97D00000;
	}else{
		fbi->screen_base =
 ...
 */
int mxcfb_probe(u32 interface_pix_fmt, struct fb_videomode *mode, int di, int is_overlay)
{
	struct fb_info *fbi;
	struct mxcfb_info *mxcfbi;
	int ret = 0;
	...
	// if we have used BG(Display Process), we can only use dc(Display Controller) instead
	// system memory can be send to DP or DC
	// when we exit from DP, we can only send data to DC
	if (!g_dp_in_use) {
		...
		g_dp_in_use = 1;
//		ipu_disp_set_global_alpha(mxcfbi->ipu_ch, 1, 0xFF);
//		ipu_disp_set_color_key(mxcfbi->ipu_ch, 1, 0xFFFF);
	} else {	// should not in this case
	...
	if (is_overlay) {	/* Overlay */
		...
		ipu_disp_set_global_alpha(mxcfbi->ipu_ch, 1, 0x0);
		ipu_disp_set_color_key(mxcfbi->ipu_ch, 1, 0xFF00FF);	
	}	
...
void lcd_ctrl_init(void *lcdbase)
{
	...
	if (!lcdbase)
		return;
	// Not must need, as we will set the bg during lcd_clear this operation can save us about 100ms in the uboot
		  ...
		  ipu_di_clk_parent_t di_clk_parent, int di_clk_val)
{
	...
	ret = mxcfb_probe(interface_pix_fmt, mode, di, 0);
