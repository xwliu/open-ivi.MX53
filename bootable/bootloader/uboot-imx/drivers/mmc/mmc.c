...
static int mmc_go_idle(struct mmc *mmc)
{
	...
	//if(mmc->dev==1)
	{
		cmd.cmdarg = 0xFFFFFFFA;
		//printf("it's iNand let send boot command\n");
	}
...
static int mmc_send_op_cond(struct mmc *mmc)
{
	...
	mmc->ocr = cmd.response[0];
	//printf("Card ocr: 0x%x\n", mmc->ocr);
	mmc->high_capacity = ((mmc->ocr & OCR_HCS) == OCR_HCS);
	//printf("Card high_capacity: %d\n", mmc->high_capacity);
...
static int mmc_change_freq(struct mmc *mmc)
{
	...
	//printf("%s mmc_send_ext_csd I  ,err=%d\n",__func__,err);
	...
	if (err)
		...
		err = mmc_send_cmd(mmc, &cmd, NULL);
		}while(!(((cmd.response[0]>>8)&1)));

	/* Now check to see that it worked */
	err = mmc_send_ext_csd(mmc, ext_csd);
	//printf("%s mmc_send_ext_csd II  ,err=%d\n",__func__,err);
...
#endif
int mmc_switch_bus_width(struct mmc *mmc, uint buswidth)
{
	uint pbuswidth;
	int err;
	struct mmc_cmd cmd;
	...
	if(buswidth==0)
	{
		...
		buswidth = EMMC_MODE_8BIT_DDR;
		else if(mmc->card_caps&EMMC_MODE_4BIT_DDR)
		buswidth = EMMC_MODE_4BIT_DDR;	
		else if(mmc->card_caps&MMC_MODE_8BIT)
		buswidth = MMC_MODE_8BIT;	
		else if(mmc->card_caps&MMC_MODE_4BIT)
		buswidth = MMC_MODE_4BIT;	
	}

	pbuswidth = buswidth;
	//for ddr mode , we aways enter 4bit mode first
	if((buswidth==EMMC_MODE_8BIT_DDR)||(buswidth==EMMC_MODE_4BIT_DDR))
	{
		pbuswidth = MMC_MODE_4BIT;
	}


	if (pbuswidth == MMC_MODE_8BIT) {
			...
			if (err)
				return err; 	
	}else if (pbuswidth == MMC_MODE_4BIT) {
	...
	mmc_set_bus_width(mmc, pbuswidth);
...
///////////////////////////////////////////////////////////////
	if((buswidth==EMMC_MODE_8BIT_DDR)||(EMMC_MODE_4BIT_DDR==buswidth))
	{
		//ddr mode steps
		//s1 set block len to 512
		mmc_set_blocklen(mmc,512);
		if (err)
			return err; 		
		//s2 set mmc clock 2 20MHz
		mmc_set_clock(mmc, 20000000);
		if (err)
			return err; 		
		//s3 set HS MODE
		err = mmc_switch(mmc, EXT_CSD_CMD_SET_NORMAL, 0xBB, 0x4);
		if (err)
			return err; 
		//s4 set power class
		err = mmc_switch(mmc, EXT_CSD_CMD_SET_NORMAL, EXT_CSD_HS_TIMING, 1);
		if (err)
			return err; 
		//s5 switch card bus width
		err = mmc_switch(mmc, EXT_CSD_CMD_SET_NORMAL,
				EXT_CSD_BUS_WIDTH,
				(buswidth==EMMC_MODE_8BIT_DDR)?EXT_CSD_BUS_WIDTH_8_DDR:EXT_CSD_BUS_WIDTH_4_DDR);		
		if (err)
			return err;  
		//s6 wait for busy
		do{
		...
		//s7 apply host bus change
		mmc_set_bus_width(mmc, buswidth);
		...
		if (mmc->card_caps & MMC_MODE_HS) {
			...
			mmc_set_clock(mmc, 20000000);
		//s8 test ext_esd  if failed ,means card timing can't mean our setting ,enter sdr mode
		if(mmc_send_ext_csd(mmc, ext_csd))
		{

			printf("!!!!!warning:card timing can't meet our setting ,enter sdr mode\n ");
			err =mmc_switch_bus_width(mmc,(buswidth==EMMC_MODE_8BIT_DDR)?MMC_MODE_8BIT:MMC_MODE_4BIT);
			if (err)
				return err; 
...
static int mmc_startup(struct mmc *mmc)
{
	...
	if (err)
		return err;
	//printf("MMC_CMD_SELECT_CARD done\n");
	...
	if (IS_SD(mmc)){
		...
		if (err)
			printf("%s sd_change_freq failed ,err=%d\n",__func__,err);
	}
	else{
		err = mmc_change_freq(mmc);
		if (err)
			printf("%s mmc_change_freq failed ,err=%d\n",__func__,err);
	}
	if (err)
		return err;
...
static int s_already_init_mmc0 = 0;
static int s_already_init_mmc1 = 0;	
static int s_already_init_mmc2 = 0;	

int mmc_init(struct mmc *mmc)
{
	int err;

	if ( (s_already_init_mmc0) && (mmc->block_dev.dev == 0))
		return 0;
	if ( (s_already_init_mmc1) && (mmc->block_dev.dev == 1))
		return 0;
	if ( (s_already_init_mmc2) && (mmc->block_dev.dev == 2))
		return 0;
	err = mmc->init(mmc);

	if (err)
		return err;
	//printf("mmc call init done!\n");
	...
	if(mmc->block_dev.dev == 0)
		s_already_init_mmc0 = 1;
	if(mmc->block_dev.dev == 1)
		s_already_init_mmc1 = 1;
	if(mmc->block_dev.dev == 2)
		s_already_init_mmc2 = 1;

	return mmc_startup(mmc);
}

void set_init_mmc_flag(int mmc_num)
{
	if(mmc_num == 0)
		s_already_init_mmc0 = 0;
	if(mmc_num == 1)
		s_already_init_mmc1 = 0;
	printf("s_already_init_mmc0 : %d\n",s_already_init_mmc0);
	printf("s_already_init_mmc1 : %d\n",s_already_init_mmc1);
}
