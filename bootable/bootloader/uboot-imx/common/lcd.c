...
int lcd_display_bitmap(ulong bmp_image, int x, int y)
{
	...
	if (bpix != bmp_bpix && (bmp_bpix != 8 || bpix != 16) && (bmp_bpix != 8 || bpix != 24)) {
#endif
		printf ("Error: %d bit/pixel mode, but BMP has %d bit/pixel\n",
			bpix,
	...
	case 8:
		if (bpix != 16)
			...
			byte_width = width * 2;
		printf ("Display Bitmap bmp_bpix: %d, byte_width: %d, height: %d pos X: 0x%x, pos Y: 0x%x\n", bmp_bpix, byte_width, height, x, y);
		for (i = 0; i < height; ++i) {
			WATCHDOG_RESET();
			for (j = 0; j < width; j++) {
				if (bpix != 16) {
