...
static void setOverlayOutput(fb_context_t* dev)
{
   ...
   if(dev->sec_disp_connected&&dev->to_disable_overlay){
		...
		if (!strcmp(value, "1")) {
			...
			if(!dev->to_tvout_display){
				property_set("sys.VIDEO_TVOUT_DISPLAY", "1");
				dev->to_tvout_display = 1;
...
static int isStartSecFramebuffer(fb_context_t* dev){
   int isStart = 0;
   ...
   if (strcmp(value, "1") == 0) {
        dev->sec_disp_connected = 1;
   }
   else{
        dev->sec_disp_connected = 0;
   }

   property_get("rw.secfb.disable-overlay", value, "0");
   if (strcmp(value, "1") == 0) {
        dev->to_disable_overlay = 1;
   }
   else{
        dev->to_disable_overlay = 0;
   }
   ...
   //in the case video to sec display, and no ui to second display
   //no init the sec framebuffer
   if(dev->sec_disp_connected&&(!(dev->video_playing&&dev->to_disable_overlay)) ) {
       isStart = 1;
   }
   return isStart;
}
    ...
    if (hnd->flags & private_handle_t::PRIV_FLAGS_FRAMEBUFFER) {
        ...
        if (isStartSecFramebuffer(ctx)) {
            ...
            if(ctx->sec_display_inited) {
                ...
                //in any time, main ui thread should not be blocked
                struct timespec wait_time={0,0};
                clock_gettime( CLOCK_REALTIME, &wait_time);
                wait_time.tv_sec += 1;//1 sec wait time
                if(sem_timedwait(&ctx->sec_display_end, &wait_time) != 0){
                    LOGE("Error! cannot wait sec_display_end sem within 1 seconds");
                }
                ...
                if(ctx->sec_fp && ctx->video_play_mode == DUAL_VIDEO_SIN_UI && ctx->sec_disp_connected == 0) {
...
int mapFrameBufferLocked(struct private_module_t* module)
{
    ...
    struct fb_var_screeninfo oldinfo;
    if (ioctl(fd, FBIOGET_VSCREENINFO, &info) == -1)
        return -errno;

    memcpy(&oldinfo, &info, sizeof(struct fb_var_screeninfo));
    info.reserved[0] = 0;
    info.reserved[1] = 0;
    info.reserved[2] = 0;
    info.xoffset = 0;
    ...
    if(memcmp(&oldinfo, &info, sizeof(struct fb_var_screeninfo))){
		LOGI("var screen info change!");
    	...
    	LOGI("var screen info already same!");
    }
	...
	LOGI("BASE:0x%x, phys:0x%x", module->framebuffer->base, module->framebuffer->phys);
...
//    set_graphics_fb_mode(1,1);
    ...
    */
    ...
    info.xres_virtual = ALIGN_PIXEL(info.xres);
#else /* Legacy TVout */
	info.nonstd = v4l2_fourcc('U', 'Y', 'V', 'Y');
#endif
...
static int resizeToSecFrameBuffer(int base,int phys,fb_context_t* ctx)
{
	...
	sIPUInputParam.fmt = v4l2_fourcc('B', 'G', 'R', '4');
	
//	LOGI("ctx->device.width:%d	ctx->device.height:%d",ctx->device.width, ctx->device.height);
//	LOGI("sIPUInputParam.user_def_paddr:0x%x", phys);
	sIPUInputParam.user_def_paddr[0] = phys;

	
	sec_width = ALIGN_PIXEL(ctx->sec_disp_w);
	//sec_width = ctx->sec_disp_w;
	sec_height = ctx->sec_disp_h;

    //Setting output format
    //Should align with v4l
	sIPUOutputParam.fmt = v4l2_fourcc('U', 'Y', 'V', 'Y');
 	sIPUOutputParam.width = sec_width;
 	sIPUOutputParam.height = sec_height;
     sIPUOutputParam.show_to_fb = 0;
     //Output param should be same as input, since no resize,crop
     sIPUOutputParam.output_win.pos.x = 20;
     sIPUOutputParam.output_win.pos.y = 0;
	sIPUOutputParam.output_win.win_w = sec_width - 64;
	sIPUOutputParam.output_win.win_h = sec_height- 4;
//	LOGI("sec_width:%d	sec_height:%d",sec_width, sec_height);
/*
    int output_w = 0;
    int output_h = 0;
        ...
        hw_device_t** device)
{
    ...
    if (!strcmp(name, GRALLOC_HARDWARE_FB0)) {
        ...
        #endif
/*
		dev->enabled_sec_disp=0;//whether software sec disp is ready for use
		dev->sec_disp_connected=0;//whether physical connected on sec disp
		dev->to_disable_overlay=0;//whether to disable overlay when video playback
		dev->to_tvout_display=0;//whether video ready to use fb1 for video on sec display
		dev->video_playing=0;
*/
        ...
        if (status >= 0) {
			...
			LOGI("xres: %d, yres: %d, stride: %d\n",m->info.xres, m->info.yres, stride);
