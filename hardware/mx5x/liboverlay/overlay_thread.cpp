...
bool OverlayThread::checkSecVideoParam(struct overlay_control_context_t *dev)
{
    ...
    if((sec_video_mIPUInputParam.width != dev->overlay_intances[0]->mHandle.width) ||
      ...
      (sec_video_mIPUInputParam.fmt != get_v4l2_fmt(dev->overlay_intances[0]->mHandle.format))) {
        ...
        }else if(m_dev->overlay_intances[0]->mHandle.format == HAL_PIXEL_FORMAT_RGB_888) {
            sec_video_mIPUInputParam.fmt = v4l2_fourcc('R', 'G', 'B', '3');
        }else{
...
bool OverlayThread::switchDualVideoSinUI(struct overlay_control_context_t * dev)
{
    ...
    sec_video_mIPUOutputParam.fb_disp.pos.x = 0;
    sec_video_mIPUOutputParam.fb_disp.pos.y = 0;
    sec_video_mIPUOutputParam.output_win.pos.x = 20;
    sec_video_mIPUOutputParam.output_win.pos.y = 0;
    sec_video_mIPUOutputParam.output_win.win_w = fb_var.xres - 48;
    sec_video_mIPUOutputParam.output_win.win_h = fb_var.yres;
    sec_video_mIPUOutputParam.rot = 0;
    sec_video_mIPUOutputParam.user_def_paddr[0] = 0;
    sec_video_mIPURet = mxc_ipu_lib_task_init(&sec_video_mIPUInputParam,NULL,&sec_video_mIPUOutputParam,OP_NORMAL_MODE|TASK_VF_MODE,&sec_video_mIPUHandle);
...
bool OverlayThread::threadLoop() {
        ...
        while(m_dev&&(m_dev->overlay_running)) {
            ...
            if(overlayObj0 || overlayObj1){
				...
				if((current_time.tv_nsec > last_check_time.tv_nsec) || (current_time.tv_sec > last_check_time.tv_sec)){
            ...
            if(overlay_buf0 && (overlayObj0->outX != 0  ||
				...
				else if(overlayObj0->mHandle.format == HAL_PIXEL_FORMAT_RGB_888) {
                    mIPUInputParam.fmt = v4l2_fourcc('R', 'G', 'B', '3');
                }else{
