...
public:
     ...
     * return the frame size (number of bytes per sample).
     */
    virtual uint32_t    frameSize() const = 0; //{ return AudioSystem::popCount(channels())*((format()==AudioSystem::PCM_16_BIT)?sizeof(int16_t):sizeof(int8_t)); }
...
public:
	...
	virtual status_t    adjustFbVolume(int chn, int vol) = 0;
	virtual status_t    adjustMasterVolume(int vol) = 0;
	...
	virtual status_t    switchSpdifRate(int rate) = 0;
	virtual status_t    getA2dpSampleRate(int *rate) = 0;
