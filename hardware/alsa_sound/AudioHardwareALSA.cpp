...
#define UDA1388  "imx-tractor"
#define PCMIN    "tractor-pcmin"
#define BT_SCO   "tractor-bt"

#define MAXCARDSNUM  5

#define MASTER_VOLUME_INDEX_DEFAULT         40
#define MASTER_VOLUME_INDEX_MAX             40
...
AudioHardwareALSA::AudioHardwareALSA() :
    ...
    mCenter(255),
    mBass(255),
    mVolumeIndex(MASTER_VOLUME_INDEX_DEFAULT),
    mSpdifDeviceInit(0)
{
    snd_lib_error_set_handler(&ALSAErrorHandler);
    hw_module_t *module;
    char snd_sgtl5000[32], snd_spdif[32], snd_uda1388[32], snd_pcmin[32], snd_bt[32];
    char **cardname = new char*[MAXCARDSNUM];

	LOGD("AudioHardwareALSA++");
    ...
    if (err == 0) {
        for (id = 0; id < MAXCARDSNUM; id++) {
            if(cardname[id] && strstr(cardname[id],SPDIF)){
				...
				//mSpdifControl = new ALSAControl("hw:2");
			...
			}else if (cardname[id] && strstr(cardname[id],UDA1388)){
				 LOGD("  CARD NAME: %s ID %d", cardname[id],id);
				 sprintf(snd_uda1388,"hw:0%d",id);
				 sprintf(snd_uda1388,"hw:CARD=%d",id);
				 mMixerUda1388 = new ALSAMixer(snd_uda1388);
			}else if (cardname[id] && strstr(cardname[id],PCMIN)){
				 LOGD("  CARD NAME: %s ID %d", cardname[id],id);
				 sprintf(snd_pcmin,"hw:0%d",id);
				 sprintf(snd_pcmin,"hw:CARD=%d",id);
				 mMixerPcmin = new ALSAMixer(snd_pcmin);
			}
			else if (cardname[id] && strstr(cardname[id],BT_SCO)){
				 LOGD("  CARD NAME: %s ID %d", cardname[id],id);
				 sprintf(snd_bt,"hw:0%d",id);
				 sprintf(snd_bt,"hw:CARD=%d",id);
				 mMixerBt = new ALSAMixer(snd_bt);
			}
        }
    } else {
		...
		mMixerUda1388 = new ALSAMixer("hw:00");
		mMixerPcmin = new ALSAMixer("hw:00");
		mMixerBt = new ALSAMixer("hw:00");
    }
    for (int i = 0; i < MAXCARDSNUM; i++) {
        delete []cardname[i];
    }
    ...
    /* set current card as Uda1388 default */
    if(mMixerUda1388)
    {
    	strcpy(mCurCard,UDA1388);
		mMixer = mMixerUda1388;
		mSoftVolControl = new ALSAControl("hw:0");
		mMasterVolControl = new ALSAControl("hw:0");
    }
    else if(mMixerSgtl5000)
    {
    ...
    {
        strcpy(mCurCard,PCMIN);
        mMixer = mMixerPcmin;        
    }
	else if(mMixerBt)
    {
        strcpy(mCurCard,BT_SCO);
        mMixer = mMixerBt;
    }
...
AudioHardwareALSA::~AudioHardwareALSA()
{
	...
	if (mMixerUda1388) delete mMixerUda1388;
	if (mMixerPcmin) delete mMixerPcmin;
	if (mMixerBt) delete mMixerBt;
...
status_t AudioHardwareALSA::adjustMasterVolume(int vol)
{
	int value = 0;
	
	LOGD("AudioHardwareALSA::adjustMasterVolume++, vol:%d", vol);
	...
	if(mVolumeIndex == 0) {
		...
		value = ((256-16)/MASTER_VOLUME_INDEX_MAX)*(mVolumeIndex) + 16;
	}

	mSoftVolControl->setFB("SoftMaster", value, value, value, value, value, value);
	
	LOGD("AudioHardwareALSA::adjustMasterVolume--");

	return NO_ERROR;
}

status_t AudioHardwareALSA::adjustFbVolume(int chn, int vol)
{
	LOGD("AudioHardwareALSA::adjustFbVolume++, chn:%d, vol:%d", chn, vol);
	int value = 0;
	int rxclk = 0;
	int channelNum = chn;
	
	mSoftVolControl->get("SoftFB", &value, 0);
	//LOGD("value:%d", value);
	...
	switch(channelNum)
	{
		case 0:
			mFL = value;
			//mSpdifControl->getSpdifRxClk("RX Sample Rate", &rxclk, 0);
			//LOGD("value:%d", rxclk);
			break;
	...
	//mSoftVolControl->set("SoftFB", value, 0);
	//mSoftVolControl->set("SoftFB", value, 1);
	//mSoftVolControl->set("SoftFB", value, 2);
	//mSoftVolControl->set("SoftFB", value, 3);
	mSoftVolControl->setFB("SoftFB", mFL, mFR, mRL, mRR, mCenter, mBass);
	
	//LOGD("AudioHardwareALSA::adjustFbVolume--");
...
status_t AudioHardwareALSA::setMode(int mode)
{
	...
	LOGV("AudioHardwareALSA::setMode++");
    if (mode != mMode) {
...
{
	LOGV("AudioHardwareALSA::getInputBufferSize++,sampleRate:%d,channels:%d", sampleRate,channels);
                                   ...
                                   AudioSystem::audio_in_acoustics acoustics)
{
	...
	alsa_handle_t * spdif_handle = 0;

	LOGV("openInputStream++, devices:0x%x, format:%d, channels:%d, sampleRate:%d, status:%d", devices, *format, *channels, *sampleRate, *status);
    if (devices & (devices - 1)) {
		LOGD("openInputStream invalid device");
        if (status) *status = err;
        return in;
    }
        ...
        it != mDeviceList.end(); ++it)
    {
		...
		{
			LOGD("openInputStream mpeg device 48000");
            err = mALSADevice->open(&(*it), devices, mode());
            if (err) break;
            if (devices & AudioSystem::DEVICE_OUT_WIRED_HDMI){
                strcpy(mCurCard ,SPDIF);
		...
		if((it->devices & devices) && 
			...
			(*sampleRate == it->sampleRate)) 
		{
			LOGV("openInputStream mpeg device 44100 1111,rate:%d", it->sampleRate);
			LOGD("openInputStream mpeg device 44100");
            err = mALSADevice->open(&(*it), devices, mode());
            if (err) break;
            if (devices & AudioSystem::DEVICE_OUT_WIRED_HDMI){
                strcpy(mCurCard ,SPDIF);
		...
		if((it->devices & devices) && 
			...
			(*sampleRate == it->sampleRate)) 
		{
			LOGD("openInputStream mpeg device 32000");
            err = mALSADevice->open(&(*it), devices, mode());
            if (err) break;
            if (devices & AudioSystem::DEVICE_OUT_WIRED_HDMI){
                strcpy(mCurCard ,SPDIF);
		...
		if((it->devices & devices) && 
			...
			(*sampleRate == it->sampleRate)) 
		{
			LOGD("openInputStream mpeg device 16000");
            err = mALSADevice->open(&(*it), devices, mode());
            if (err) break;
            if (devices & AudioSystem::DEVICE_OUT_WIRED_HDMI){
                strcpy(mCurCard ,SPDIF);
		...
		if((it->devices & devices) && 
			...
			(*sampleRate == it->sampleRate)) 
		{
			LOGD("openInputStream mpeg device 8000");
            err = mALSADevice->open(&(*it), devices, mode());
            if (err) break;
            if (devices & AudioSystem::DEVICE_OUT_WIRED_HDMI){
                strcpy(mCurCard ,SPDIF);
        ...
        if ((it->devices & devices) && !(devices & AudioSystem::DEVICE_IN_MPEG)) 
		{
			LOGD("openInputStream other than mepg");
            err = mALSADevice->open(&(*it), devices, mode());
            if (err) break;
            if (devices & AudioSystem::DEVICE_OUT_WIRED_HDMI){
                strcpy(mCurCard ,SPDIF);
...
status_t AudioHardwareALSA::switchSpdifRate(int rate)
{
	status_t err = NO_ERROR;
	alsa_handle_t *handle;
	
	LOGV("switchSpdifRate, rate:%d", rate);

    // Find the appropriate alsa device
    for(ALSAHandleList::iterator it = mDeviceList.begin(); it != mDeviceList.end(); ++it)
    {
		if ((it->devices & AudioSystem::DEVICE_IN_MPEG) && 
			(rate == it->sampleRate)) 
		{
			LOGV("switchSpdifRate, it->sampleRate:%d", it->sampleRate);
	        err = mALSADevice->switchSpdifRate(&(*it), AudioSystem::DEVICE_IN_MPEG, rate);
	        if (err) break;
    ...
    for(ALSAHandleList::iterator it = mDeviceList.begin(); it != mDeviceList.end(); ++it)
    {
		if (it->devices & AudioSystem::DEVICE_IN_MPEG)		
		{
			LOGV("switchSpdifRate, sync handle");
...
status_t AudioHardwareALSA::getA2dpSampleRate(int *rate)
{
    return NO_ERROR;
}
