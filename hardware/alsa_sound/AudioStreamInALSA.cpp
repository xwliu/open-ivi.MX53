...
ssize_t AudioStreamInALSA::read(void *buffer, ssize_t bytes)
{
    ...
    status_t          err;
	//LOGD("AudioStreamInALSA::read, frames:0x%x, bytes:0x%x", frames, bytes);
    do {
        if (mHandle->mmap)
        ...
        if (n < frames) {
            if (mHandle->handle) {
                if (n < 0) {
					LOGW("overrun and do recovery....., n:%d", (int)n);
                    n = snd_pcm_recover(mHandle->handle, n, 0);

                    if (aDev && aDev->recover) aDev->recover(aDev, n);
                } else {
                	LOGW("overrun n:%d", (int)n);
                    n = snd_pcm_prepare(mHandle->handle);
                }
            }
			//LOGD("AudioStreamInALSA::read, n2:%d", n);
            return static_cast<ssize_t>(n);
        }
    } while (n == -EAGAIN);
	//LOGD("AudioStreamInALSA::read--, n:%d", n);
