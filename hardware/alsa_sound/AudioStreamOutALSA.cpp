...
uint32_t AudioStreamOutALSA::channels() const
{
	...
	LOGV("$$AudioStreamOutALSA::channels,channels:0x%x",c);
    return c;
}
...
ssize_t AudioStreamOutALSA::write(const void *buffer, size_t bytes)
{
	...
	//LOGD("$$AudioStreamOutALSA::write,bytes:%d",bytes);
    if (!mPowerLock) {
