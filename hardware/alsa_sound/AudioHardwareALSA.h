...
struct alsa_device_t {
    ...
    status_t (*switchSpdifRate)(alsa_handle_t *, uint32_t, int);
};
...
public:
	...
	status_t                getSpdifRxClk(const char *name, int *value, int index = 0);
	
    status_t                set(const char *name, unsigned int value, int index = -1);

    status_t                set(const char *name, const char *);

	status_t                setFB(const char *name, unsigned int FL, unsigned int FR, unsigned int RL, unsigned int RR, unsigned int center, unsigned int bass);
...
public:
    ...
    {
        return ALSAStreamOps::frameSize();
    }
...
public:
	...
	virtual status_t    adjustFbVolume(int chn, int vol);
	virtual status_t    adjustMasterVolume(int vol);
	...
	virtual status_t switchSpdifRate(int rate);
	virtual status_t getA2dpSampleRate(int *rate);
...
protected:
	...
	ALSAMixer *         mMixerUda1388;
	ALSAMixer *         mMixerPcmin;
	ALSAMixer *         mMixerBt;

	ALSAControl *       mSoftVolControl;
	int                 mFL;
	int                 mFR;
	int                 mRL;
	int                 mRR;
	int                 mCenter;
	int                 mBass;
	ALSAControl *       mMasterVolControl;
	int                 mVolumeIndex;
	
	ALSAControl *       mSpdifControl;
