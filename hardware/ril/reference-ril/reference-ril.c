...
static int s_modem_model = UNKNOWN_MODEL;
#endif
                ...
                void *data, size_t datalen, RIL_Token t)
{
    ...
    return;
error:
	//fake for unsupport command
	if(s_modem_model == CDMA_MODEL && (isRadioOn() > 0)){
		LOGE("[CDMA_MODEL]: requestQueryNetworkSelectionMode");
		int response = 0;
		RIL_onRequestComplete(t, RIL_E_SUCCESS, &response, sizeof(int));
	    at_response_free(p_response);
		return;
...
static void requestGetCurrentCalls(void *data, size_t datalen, RIL_Token t)
{
	...
	if ((err != 0 || p_response->success == 0) && s_modem_model == CDMA_MODEL && (isRadioOn() > 0)){
		LOGE("fake CLCC response");
		err = 0;
		p_response->success = 1;
		strcpy(p_response->finalResponse, "OK");
...
static int getRegistrationNetworkType(void)
{
    int err;
    ...
    int commas;
    int skip;
	int i, networkType = DATA_ACCESS_UNKNOWN;
    ...
    if (err != 0 || p_response->success == 0) goto second;

    line = p_response->p_intermediates->line;

    err = at_tok_start(&line);
    if (err < 0) goto second;

	/* Ok you have to be careful here
	 * The solicited version of the SYSINFO response is
	 * ^SYSINFO:<srv_status>, <srv_domain>, <roam_status>,
	 *		<sys_mode>, <sim_state> [,<reserved>,<sys_submode>]
	 *
	...
	for(i = 0; i <= commas; i++){
		at_tok_nextint(&line, &response[i]);
		//LOGW("3G Modem response[%d]: %d", i, response[i]);
		if (3 == i) { /* sys_mode */
			switch (response[i]) {
				case 0: /* no server */
					networkType = DATA_ACCESS_UNKNOWN;
					break;
					
				case 5: /* WCDMA mode */
				case 15: /* TD_SCDMA mode */
					networkType = DATA_ACCESS_UMTS;
					break;

				case 2: /* CDMA 1x mode */
					networkType = DATA_ACCESS_CDMA_1xRTT;
					break;

				case 4: /* CDMA EVDO mode */
				case 8:
					networkType = DATA_ACCESS_CDMA_EvDo_A;
					break; 
			...
			LOGW("SYSINFO sys_mode: %d", networkType);
		}else if (6 == i && s_modem_model != CDMA_MODEL) { /*sys_submode*/
			switch (response[i]) {
				case 0: /* no server */
					networkType = DATA_ACCESS_UNKNOWN;
					break;
				...
				case 1: /* GSM mode */
				case 2:
					networkType = DATA_ACCESS_GPRS;
					break;
					
				case 3: /* EDGE mode */
					networkType = DATA_ACCESS_EDGE;
					break;

				case 8: /* TD_SCDMA mode */
				case 4: /* WCDMA mode */
					networkType = DATA_ACCESS_UMTS;
					break;

				case 5: /* HSDPA mode */
					networkType = DATA_ACCESS_HSDPA;
					break;

				case 6: /* HSUPA mode*/
					networkType = DATA_ACCESS_HSUPA;
					break;

				case 7: /* HSDPA mode and HSUPA mode */
					networkType = DATA_ACCESS_HSUPA;
					break;
			...
			LOGW("SYSINFO sys_submode: %d", networkType);
		}
	}
...
second:
	...
	/* ZTE private AT command
	 * The solicited version of the ZPAS response is
	 * +ZPAS: <network>, <srv_domain>
	...
	if (DATA_ACCESS_UNKNOWN == networkType && ZTE_MODEM == current_modem_type) {
		char *out;
		cmd = "AT+ZPAS?";
	    prefix = "+ZPAS:";
		...
		LOGE("ZTE network: %s", out);
		if (0 == strcmp (out, "EDGE")) {
			networkType = DATA_ACCESS_EDGE;
		}else if (0 == strcmp (out, "GPRS")) {
			networkType = DATA_ACCESS_GPRS;
		}else if (0 == strcmp (out, "HSDPA")) {
			networkType = DATA_ACCESS_HSDPA;
		}else if (0 == strcmp (out, "HSUPA")) {
			networkType = DATA_ACCESS_HSUPA;
		}else if (0 == strcmp (out, "UMTS")) {
			networkType = DATA_ACCESS_UMTS;
		}else {
			networkType = DATA_ACCESS_UNKNOWN;
		}
		...
		//at_tok_nextstr(&line, &out);
		//LOGE("ZTE srv_domain: %s", out);

	}

done:

	LOGW("3G Modem networkType: %d", networkType);
    at_response_free(p_response);
	return networkType;
}

static void requestRegistrationNetworkType(int request, void *data,
                                        size_t datalen, RIL_Token t)
{
  	char * responseStr[4];
	int networkType = getRegistrationNetworkType();
    asprintf(&responseStr[0], "%d", 1);
    asprintf(&responseStr[1], "%x", 0xFF);
    asprintf(&responseStr[2], "%x", 0xFF);
	asprintf(&responseStr[3], "%x", networkType);
...
error:
	//fake for unsupport command
	if(s_modem_model == CDMA_MODEL && (isRadioOn() > 0)){
		LOGE("[CDMA_MODEL]: requestOperator");
		char *response[3] = {"CHN-TELECOM", "TELECOM", "46003"};
		RIL_onRequestComplete(t, RIL_E_SUCCESS, response, sizeof(response));
	    at_response_free(p_response);
		return;
	}
...
static void requestSetupDataCall(void *data, size_t datalen, RIL_Token t)
{
    ...
    if (fd >= 0) { /* the device doesn't exist on the emulator */
	    ...
	    err = at_send_command("AT+CGEREP=1,0", NULL);
	     ...
	     if (current_modem_type == ZTE_MODEM)
		 // Hangup anything that's happening there now
	 	err = at_send_command("AT+CGACT=1", NULL);
	    else
	    // Hangup anything that's happening there now
	...
	if(CDMA_MODEL == s_modem_model){
		err = at_send_command("AT", &p_response);
		at_send_command("ATZ", NULL);
		at_send_command("ATQ0 V1 E1 S0=0 &C1 &D2 +FCLASS=0", NULL);
		//at_send_command("ATDT#777", NULL);
    }else if (current_modem_type != AMAZON_MODEM) {
	    /* Hangup anything that's happening there now */
	    err = at_send_command("AT+CGACT=0,1", &p_response);

	    /* Start data on PDP context 1 */
		if (TD_SCDMA_MODEL == s_modem_model && HUAWEI_MODEM == current_modem_type)
...
onRequest (int request, void *data, size_t datalen, RIL_Token t)
{
    ...
    switch (request) {
        ...
        case RIL_REQUEST_GPRS_REGISTRATION_STATE:	
            ...
            /* For USB 3G modem */
			requestRegistrationNetworkType(request, data, datalen, t);
            break;
...
getSIMStatus()
{
    ...
    if (0 == strcmp (cpinResult, "SIM PIN")) {
    	...
    	if(NULL != strstr (cpinResult, "READY")){
			/* 	Contains of some needless information in ZTE AC591.
	        	 *	We should support it.	
	        	 */
        	 
	       	LOGW("getSIMStatus, cpinResult: %s", cpinResult);
		}else{
...
static void pollNetState(int time){
    int i = 0, err, skip;
	char *line;
    const char *cmd;
    const char *prefix;
    ...
    if(time < 0)
        time = 0;
    int counter = time * (1000 / POLLING_DELAY);
	LOGE("3G Modem Network state check with %d ms.", time);
    for(i = 0 ; i < counter; i++){
        if (DATA_ACCESS_UNKNOWN != getRegistrationNetworkType())
			break;
...
#endif
		s_modem_model = runtime_3g_modem_model();
        write_runtime_conf(s_data_device_path, s_modem_model);
            if (s_port > 0) {
