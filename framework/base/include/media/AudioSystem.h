...
#include <media/ICarAudioService.h>

namespace android {

typedef void (*audio_error_callback)(status_t err);
typedef void (*track_status_change_callback)(int32_t pid, int32_t status);

typedef int audio_io_handle_t;

class IAudioPolicyService;
class ICarAudioService;
class String8;
...
public:
    ...
    enum stream_type {
		...
		//twls ivi audio system extended
		STREAM_GPS = 10,
		STREAM_FM = 11,
		STREAM_MPEG = 12,
		STREAM_CMMB = 13,
    ...
    enum audio_format {
        ...
        PCM_20_BIT         = (PCM|PCM_SUB_20_BIT),
        PCM_24_BIT         = (PCM|PCM_SUB_24_BIT)
    };
	...
	static status_t switchSpdifRate(int rate);
	static status_t getA2dpSampleRate(int *rate);
	
    // set/get master volume
    static status_t setMasterVolume(float value);
    static status_t getMasterVolume(float* volume);

	static status_t adjustFbVolume(int chn, int vol);
	static status_t adjustMasterVolume(int vol);
	...
	static void setTrackStatusChangeCallback(track_status_change_callback cb);
	...
	DEVICE_OUT_TTY = 0x2000,
        ...
        DEVICE_IN_BLUETOOTH_SCO_HEADSET = 0x80000,    //use this as a2dp, because bitmask is not enough now
        DEVICE_IN_WIRED_HEADSET = 0x100000,
        DEVICE_IN_AUX_DIGITAL = 0x200000,
        DEVICE_IN_VOICE_CALL = 0x400000,
        DEVICE_IN_BACK_MIC = 0x800000,
		DEVICE_IN_FM = 0x1000000,
		DEVICE_IN_MPEG = 0x2000000,
		DEVICE_IN_CMMB = 0x4000000,
		DEVICE_IN_AUX1 = 0x8000000,
		DEVICE_IN_AUX2 = 0x10000000,
		DEVICE_IN_BLUETOOTH_SCO = 0x20000000,
		DEVICE_IN_IPOD = 0x40000000,
		
        DEVICE_IN_DEFAULT = 0x80000000,

        DEVICE_IN_ALL = (DEVICE_IN_COMMUNICATION | DEVICE_IN_AMBIENT | DEVICE_IN_BUILTIN_MIC |
                DEVICE_IN_BLUETOOTH_SCO_HEADSET | DEVICE_IN_WIRED_HEADSET | DEVICE_IN_AUX_DIGITAL |
                DEVICE_IN_VOICE_CALL | DEVICE_IN_BACK_MIC | DEVICE_IN_FM | DEVICE_IN_MPEG | DEVICE_IN_CMMB | 
                DEVICE_IN_AUX1 | DEVICE_IN_AUX2 | DEVICE_IN_IPOD | DEVICE_IN_DEFAULT)
    };
	...
	static const sp<ICarAudioService>& get_car_audio_service();

	static status_t setFMChannel(bool on);
	static status_t setRoute(int channel, bool on);
	static int getIVIAudioLibVersionH();
	static int getIVIAudioLibVersionL();
	static int adjustHardwareVolume(int directions);
	static int muteAll(int mute);
	static int popLess(int mute);
	static int shutdownMute(int mute);
	static int isMute();
	static int muteMic(int mute);
	static int isMicMute();
	static int a2dpHolding(int holding);
...
private:
    ...
    class CarAudioServiceClient: public IBinder::DeathRecipient
    {
    public:
        CarAudioServiceClient() {
        }
	...
	static sp<CarAudioServiceClient> gCarAudioServiceClient;
	...
	static track_status_change_callback gTrackStatusChangeCallback;
	...
	static sp<ICarAudioService> gCarAudioService;
