...
public class PackageInstallerActivity extends Activity implements OnCancelListener, OnClickListener {
    ...
    private static final int DLG_HOME_INSTALL_FORBIDDEN = DLG_BASE + 6;
    ...
    public Dialog onCreateDialog(int id, Bundle bundle) {
        ...
        case DLG_PACKAGE_ERROR :
            return new AlertDialog.Builder(this)
                    ...
                    .setOnCancelListener(this)
                    .create();
        case DLG_HOME_INSTALL_FORBIDDEN :
            return new AlertDialog.Builder(this)
                    .setTitle(R.string.Parse_home_forbiddedn)
                    .setMessage(R.string.Parse_home_forbiddedn_text)
                    .setPositiveButton(R.string.ok, new DialogInterface.OnClickListener() {
                        public void onClick(DialogInterface dialog, int which) {
                            finish();
                        }
    ...
    protected void onCreate(Bundle icicle) {
        ...
        int N = mPkgInfo.activities.size();
        for (int i=0; i<N; i++) {
            PackageParser.Activity a = mPkgInfo.activities.get(i);
            int NI = a.intents.size();
            for(int j = 0; j < NI; j++){
                PackageParser.ActivityIntentInfo MyIntent = a.intents.get(j);
                if(MyIntent.hasCategory("android.intent.category.HOME")){
                    Log.e(TAG, "Install New Home Application Not Allowed");
                    showDialogInner(DLG_HOME_INSTALL_FORBIDDEN);
                    return;
                }
            }
        }  
