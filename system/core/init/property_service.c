...
// define TRACTOR platform special define which will be copy from burning time
#define PROP_PATH_TRACTOR_DEFAULT   "/firmware/tractor.prop"
...
struct {
    ...
    { "usb_modeswitch", AID_RADIO, AID_RADIO },
	{ "Rdisk", AID_RADIO, AID_RADIO },
    { NULL, 0, 0 }
};
...
static int check_perms(const char *name, unsigned int uid, unsigned int gid)
{
    ...
    for (i = 0; property_perms[i].prefix; i++) {
		...
		else if((strcmp("rw.SECOND_DISPLAY_CONNECTED", name) == 0) && (uid == AID_SYSTEM)) {
			return 1;
		}
    }
    ...
    case PROP_MSG_SETPROP:
        ...
        if(memcmp(msg.name,"ctl.",4) == 0) {
            ...
            if (check_perms(msg.name, cr.uid, cr.gid)) {
                ...
                ERROR("sys_prop: permission denied uid:%d  name:%s process: %d\n",
                      cr.uid, msg.name, cr.pid);
            }
        }
        break;
...
static void load_properties_from_file(const char *fn)
{
    char *data;
    unsigned sz;
    INFO("load_properties_from_file %s\n", fn);
...
void start_property_service(void)
{
    ...
    load_properties_from_file(PROP_PATH_TRACTOR_DEFAULT);    
