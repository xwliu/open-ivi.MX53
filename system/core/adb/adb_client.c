...
static int switch_socket_transport(int fd)
{
    ...
    printf("Switch transport in progress\n");
    ...
    printf("Switch transport success\n");
    return 0;
}
...
int adb_status(int fd)
{
    ...
    if(readx(fd, buf, 4)) {
        ...
        printf("protocol fault (no status)");		
        return -1;
    }
    ...
    if(memcmp(buf, "FAIL", 4)) {
        printf("protocol fault (status %02x %02x %02x %02x?!)", buf[0], buf[1], buf[2], buf[3]);		
        return -1;
    }

    if(readx(fd, buf, 4)) {
        strcpy(__adb_error, "protocol fault (status len)");
        printf("protocol fault (status len)");
        return -1;
    }
    buf[4] = 0;
    ...
    if(readx(fd, __adb_error, len)) {
        ...
        printf("protocol fault (status read)");		
        return -1;
    }
    __adb_error[len] = 0;
    return -1;
...
int _adb_connect(const char *service)
{
    ...
    if (memcmp(service,"host",4) != 0 && switch_socket_transport(fd)) {
        printf("memcmp host string failed\n");						
        return -1;
    }

    if(writex(fd, tmp, 4) || writex(fd, service, len)) {
        strcpy(__adb_error, "write failure during connection");
        printf("write failure during connection");						
        adb_close(fd);
        return -1;
    }
...
int adb_connect(const char *service)
{
    ...
    if(fd == -2) {
        printf("* daemon not running. starting it now on port %d *\n",
                __adb_server_port);
    start_server:
...
char *adb_query(const char *service)
{
    ...
    printf("adb_query: %s\n", service);
    if(readx(fd, buf, 4)) {
	printf("adb_query result error\n");
	goto oops;
    }

    printf("adb_query: buf[0-4]: %d, %d, %d, %d\n", buf[0], buf[1], buf[2], buf[3]);

    buf[4] = 0;
    n = strtoul(buf, 0, 16);
    if(n > 1024) goto oops;
    printf("adb_query: buffer number: %d\n", n);
    tmp = malloc(n + 1);
    if(tmp == 0) goto oops;

    if(readx(fd, tmp, n) == 0) {
        tmp[n] = 0;
	 printf("adb_query: result: %s\n", tmp);	
        adb_close(fd);
        return tmp;
    }
    free(tmp);
