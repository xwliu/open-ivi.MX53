...
int  adb_open(const char*  path, int  options)
{
    ...
    printf( "adb_open: path '%s' option: %d\n", path, options);
    switch (options) {
        case O_RDONLY:
        ...
        case O_RDWR:
            ...
            break;
        default:
            printf("adb_open: invalid options (0x%0x)\n", options);
            errno = EINVAL;
            return -1;
    }
    ...
    if ( f->fh_handle == INVALID_HANDLE_VALUE ) {
        _fh_close(f);
        printf( "adb_open: could not open '%s':", path );
        switch (GetLastError()) {
...
int  adb_creat(const char*  path, int  mode)
{
    ...
    if ( f->fh_handle == INVALID_HANDLE_VALUE ) {
        _fh_close(f);
        printf( "adb_creat: could not open '%s':", path );
        switch (GetLastError()) {
            case ERROR_FILE_NOT_FOUND:
                printf( "file not found\n" );
                errno = ENOENT;
...
int socket_loopback_client(int port, int type)
{
    ...
    if(s == INVALID_SOCKET) {
        printf("socket_loopback_client: could not create socket\n" );
        _fh_close(f);
        return -1;
    }
    ...
    if(connect(s, (struct sockaddr *) &addr, sizeof(addr)) < 0) {
        printf("socket_loopback_client: could not connect to %s:%d, ERROR: 0x%x\n", type != SOCK_STREAM ? "udp" : "tcp", port, WSAGetLastError() );
        _fh_close(f);
        return -1;
    }
    snprintf( f->name, sizeof(f->name), "%d(lo-client:%s%d)", _fh_to_int(f), type != SOCK_STREAM ? "udp:" : "", port );
    printf( "socket_loopback_client: port %d type %s => fd %d\n", port, type != SOCK_STREAM ? "udp" : "tcp", _fh_to_int(f) );
    return _fh_to_int(f);
}
...
int socket_loopback_server(int port, int type)
{
    ...
    if(bind(s, (struct sockaddr *) &addr, sizeof(addr)) < 0) {
	 printf("bind socket failed type: %d , port: %d\n", port, type);
        _fh_close(f);
        return -1;
    }
    if (type == SOCK_STREAM) {
    ...
    printf( "socket_loopback_server: port %d type %s => fd %d\n", port, type != SOCK_STREAM ? "udp" : "tcp", _fh_to_int(f) );
    return _fh_to_int(f);
}
