...
int launch_server(int server_port)
{
    ...
    if (!ret) {
        printf("CreateProcess failure, error %ld\n", GetLastError() );
    ...
    /* wait for the "OK\n" message */
    {
        ...
        CloseHandle( pipe_read );
        if ( !ret ) {
            printf("could not read ok from ADB Server, error = %ld\n", GetLastError() );
            return -1;
        }
        if (count != 3 || temp[0] != 'O' || temp[1] != 'K' || temp[2] != '\n') {
            printf("ADB server didn't ACK\n" );
            return -1;
        }
    }
    ...
    if (pipe(fd)) {
        printf("pipe failed in launch_server, errno: %d\n", errno);
        return -1;
    }
    ...
    if (pid == 0) {
        ...
        printf("OOPS! execl returned %d, errno: %d\n", result, errno);
    } else  {
        ...
        adb_close(fd[0]);
        if (ret < 0) {
            printf("could not read ok from ADB Server, errno = %d\n", errno);
            return -1;
        }
...
#else
    printf("adb_main device for adbd\n");    
