...
void DirectVolume::handleDiskAdded(const char *devpath, NetlinkEvent *evt) {
    ...
    if (tmp) {
        ...
        SLOGD("Kernel block uevent missing 'NPARTS'");
        mDiskNumParts = 1;
    }
    ...
    if(!tmp) {
		SLOGD("$$$no partition, send broadcast");
...
void DirectVolume::handleDiskRemoved(const char *devpath, NetlinkEvent *evt) {
    ...
    if(mDiskNumParts == 0) {
		SLOGD("$$$handleDiskRemoved, special case for disk with no partitions but valid");
