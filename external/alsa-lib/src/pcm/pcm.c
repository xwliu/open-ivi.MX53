...
{
	//SNDERR("$$$snd_pcm_wait++, pcm->stream:0x%x", pcm->stream);
...
int snd_pcm_wait_nocheck(snd_pcm_t *pcm, int timeout)
{
	...
	//SNDERR("$$$snd_pcm_wait_nocheck 111");
	if (err < 0)
		return err;
	if (err != npfds) {
	...
	do {
		//SNDERR("$$$snd_pcm_wait_nocheck 222");
		...
		//SNDERR("$$$snd_pcm_wait_nocheck 333");
...
#if 1 /* very useful code to test poll related problems */
	{
		...
		if (avail_update < (snd_pcm_sframes_t)pcm->avail_min) {
			SNDERR("*** snd_pcm_wait() FATAL ERROR!!!\n");
			SNDERR("avail_min = %li, avail_update = %li\n", pcm->avail_min, avail_update);

			//possible driver has no data,
			return -1000;
		}
	}
#endif
				     ...
				     snd_pcm_xfer_areas_func_t func)
{
	...
	//SNDERR("$$$snd_pcm_read_areas++, size:0x%x", size);
	...
	_again:
		...
		//SNDERR("$$$snd_pcm_read_areas 111");
		avail = snd_pcm_avail_update(pcm);
		if (avail < 0) {
			err = avail;
			goto _end;
		}
		//SNDERR("$$$snd_pcm_read_areas 222, avail:%d", avail);
		if (avail == 0) {
			...
			//this wail will hang if driver stop, and user buffer is not satisfied, so set a timeout, and always break if timeout reach
				      ...
				      snd_pcm_xfer_areas_func_t func)
{
	...
	//SNDERR("$$$snd_pcm_write_areas++, pcm:0x%x", pcm);
	if (size == 0)
		return 0;

	while (size > 0) {
		//SNDERR("$$$snd_pcm_write_areas 111");
		snd_pcm_uframes_t frames;
		snd_pcm_sframes_t avail;
	_again:
		state = snd_pcm_state(pcm);
		//SNDERR("$$$snd_pcm_write_areas state:0x%x", state);
		switch (state) {
		case SND_PCM_STATE_PREPARED:
		case SND_PCM_STATE_PAUSED:
			break;
		...
		//SNDERR("$$$snd_pcm_write_areas 222");
		if ((state == SND_PCM_STATE_RUNNING &&
			...
			//SNDERR("$$$snd_pcm_write_areas 222 111");
			err = snd_pcm_wait(pcm, -1);
			//SNDERR("$$$snd_pcm_write_areas 222 222");
			if (err < 0)
				break;
			goto _again;			
		}
		//SNDERR("$$$snd_pcm_write_areas 333");
		frames = size;
		if (frames > (snd_pcm_uframes_t) avail)
			frames = avail;
		if (! frames)
		...
		//SNDERR("$$$snd_pcm_write_areas 444");
		offset += frames;
		size -= frames;
		xfer += frames;
	}
