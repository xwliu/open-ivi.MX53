				       ...
				       snd_pcm_uframes_t frames)
{
	...
	//SNDERR("softvol_convert_stereo_vol++");
...
/* 6-channel stereo control */
static void softvol_convert_6channel_vol(snd_pcm_softvol_t *svol,
				       const snd_pcm_channel_area_t *dst_areas,
				       snd_pcm_uframes_t dst_offset,
				       const snd_pcm_channel_area_t *src_areas,
				       snd_pcm_uframes_t src_offset,
				       ...
				       snd_pcm_uframes_t frames)
{
	const snd_pcm_channel_area_t *dst_area, *src_area;
	unsigned int src_step, dst_step;
	unsigned int vol_scale, vol[6], vol_c;
	unsigned int ch, fr; 
	short *src, *dst; 

	//SNDERR("softvol_convert_6channel_vol++, svol->sformat:%d", svol->sformat);
	if (svol->cur_vol[0] == 0 && svol->cur_vol[1] == 0
		&& svol->cur_vol[2] == 0 && svol->cur_vol[3] == 0
		&& svol->cur_vol[4] == 0 && svol->cur_vol[5] == 0) {
		snd_pcm_areas_silence(dst_areas, dst_offset, channels, frames,
				      svol->sformat);
		return;
	} else if (svol->zero_dB_val && \
		   svol->cur_vol[0] == svol->zero_dB_val &&
		   svol->cur_vol[1] == svol->zero_dB_val &&
		   svol->cur_vol[2] == svol->zero_dB_val &&
		   svol->cur_vol[3] == svol->zero_dB_val &&
		   svol->cur_vol[4] == svol->zero_dB_val &&
		   svol->cur_vol[5] == svol->zero_dB_val) {
		snd_pcm_areas_copy(dst_areas, dst_offset, src_areas, src_offset,
				   channels, frames, svol->sformat);
		//SNDERR("softvol_convert_6channel_vol 111");
		return;
	}
	...
	if (svol->max_val == 1) {
		vol[0] = svol->cur_vol[0] ? 0xffff : 0;
		vol[1] = svol->cur_vol[1] ? 0xffff : 0;
		vol[2] = svol->cur_vol[2] ? 0xffff : 0;
		vol[3] = svol->cur_vol[3] ? 0xffff : 0;
		vol[4] = svol->cur_vol[4] ? 0xffff : 0;
		vol[5] = svol->cur_vol[5] ? 0xffff : 0;
	} else {
		vol[0] = svol->dB_value[svol->cur_vol[0]];
		vol[1] = svol->dB_value[svol->cur_vol[1]];
		vol[2] = svol->dB_value[svol->cur_vol[2]];
		vol[3] = svol->dB_value[svol->cur_vol[3]];
		vol[4] = svol->dB_value[svol->cur_vol[4]];
		vol[5] = svol->dB_value[svol->cur_vol[5]];
	}
	switch (svol->sformat) {
	case SND_PCM_FORMAT_S16_LE:
	case SND_PCM_FORMAT_S16_BE:
		...
		//CONVERT_AREA(short, 
		//	     !snd_pcm_format_cpu_endian(svol->sformat));
			//SNDERR("$$$CONVERT_AREA, channels:%d", channels); 
			for (ch = 0; ch < channels; ch++) { 
				src_area = &src_areas[ch]; 
				dst_area = &dst_areas[ch]; 
				src = snd_pcm_channel_area_addr(src_area, src_offset); 
				...
				//SNDERR("ch:%d, vol_scale:%d, vol[0]:%d, vol[1]:%d, vol[2]:%d, vol[3]:%d, vol[4]:%d, vol[5]:%d", ch, vol_scale, vol[0], vol[1], vol[2], vol[3], vol[4], vol[5]); 
				fr = frames;
				if (! vol_scale) { 
					while (fr--) { 
						*dst = 0; 
					...
					while (fr--) { 
						*dst = (short) MULTI_DIV_short(*src, vol_scale, !snd_pcm_format_cpu_endian(svol->sformat)); 
						src += src_step; 
						dst += dst_step; 
					} 
				} 
				     ...
				     snd_pcm_uframes_t frames)
{
	...
	//SNDERR("softvol_convert_mono_vol++");
	if (svol->cur_vol[0] == 0) {
		snd_pcm_areas_silence(dst_areas, dst_offset, channels, frames,
				      svol->sformat);
		return;
...
static void get_current_volume(snd_pcm_softvol_t *svol)
{
	...
	for (i = 0; i < svol->cchannels; i++) {
		...
		if (val > svol->max_val)
			val = svol->max_val;
		svol->cur_vol[i] = val;
		//svol->cur_vol[i] = svol->max_val;
	}
	//SNDERR("get_current_volume, channels:%d, 0:%d, 1:%d, 2:%d, 3:%d, 4:%d, 5:%d", svol->cchannels, svol->cur_vol[0], svol->cur_vol[1], svol->cur_vol[2], 
		//svol->cur_vol[3], svol->cur_vol[4], svol->cur_vol[5]);
}
					      ...
					      snd_pcm_hw_params_t *params)
{
	...
	//SNDERR("snd_pcm_softvol_hw_refine_cprepare++");
...
{
	//SNDERR("snd_pcm_softvol_hw_refine_sprepare++");
	snd_pcm_softvol_t *svol = pcm->private_data;
...
{
	//SNDERR("$$$snd_pcm_softvol_hw_refine");
...
static int snd_pcm_softvol_hw_params(snd_pcm_t *pcm, snd_pcm_hw_params_t * params)
{
	...
	//SNDERR("$$$snd_pcm_softvol_hw_params");
			    ...
			    snd_pcm_uframes_t *slave_sizep)
{
	...
	//SNDERR("snd_pcm_softvol_write_areas, pcm->channels:%d, svol->cchannels:%d", pcm->channels, svol->cchannels);
	if (svol->cchannels == 1)
		softvol_convert_mono_vol(svol, slave_areas, slave_offset,
					 areas, offset, pcm->channels, size);
	else if(svol->cchannels == 2)
		softvol_convert_stereo_vol(svol, slave_areas, slave_offset,
					   areas, offset, pcm->channels, size);
	else if(svol->cchannels == 6)
		softvol_convert_6channel_vol(svol, slave_areas, slave_offset,
					   areas, offset, pcm->channels, size);
	else
		softvol_convert_stereo_vol(svol, slave_areas, slave_offset,
					   areas, offset, pcm->channels, size);		
...
static int add_user_ctl(snd_pcm_softvol_t *svol, snd_ctl_elem_info_t *cinfo, int count)
{
	...
	//SNDERR("add_user_ctl++, count:%d, svol->max_val:%d", count, svol->max_val);
	...
	//SNDERR("def_val:%d, count:%d", def_val, count);
	for (i = 0; i < count; i++)
		svol->elem.value.integer.value[i] = def_val;
	return snd_ctl_elem_write(svol->ctl, &svol->elem);
#if 0
	svol->elem.value.integer.value[0] = 200;
	svol->elem.value.integer.value[1] = 201;
	svol->elem.value.integer.value[2] = 202;
	svol->elem.value.integer.value[3] = 203;

	snd_ctl_elem_write(svol->ctl, &svol->elem);
	
    err = snd_ctl_elem_read(svol->ctl, &svol->elem);
    if (err < 0) {
        SNDERR("Control cannot read element value: %d", err);
        return err;
    }

	int *pdata=(int*)snd_ctl_elem_value_get_bytes(&svol->elem);
	SNDERR("after changing 111, :%d, %d, %d, %d\n",*pdata,*(pdata+1), *(pdata+2), *(pdata+3));
	
	snd_ctl_elem_value_set_integer(&svol->elem, 0, 210);
	snd_ctl_elem_value_set_integer(&svol->elem, 1, 211);
	snd_ctl_elem_value_set_integer(&svol->elem, 2, 212);
	snd_ctl_elem_value_set_integer(&svol->elem, 3, 213);
	
	if ((err = snd_ctl_elem_write(svol->ctl, &svol->elem)) < 0) {
	...
	*pdata=(int*)snd_ctl_elem_value_get_bytes(&svol->elem);
	SNDERR("after changing 222, :%d, %d, %d, %d\n",*pdata,*(pdata+1), *(pdata+2), *(pdata+3));
				...
				int resolution)
{
	...
	//SNDERR("softvol_load_control, cchannels:%d, min_dB:%Lf, max_dB:%Lf, resolution:%d", cchannels, min_dB, max_dB, resolution);
	...
	//SNDERR("softvol_load_control open CTL %s", tmp_name);
	svol->elem.id = *ctl_id;
			 ...
			 snd_pcm_t *slave, int close_slave)
{
	...
	//SNDERR("snd_pcm_softvol_open++, name:%s, ctl_card:%d, sformat:%d", name, ctl_card, sformat);
			  ...
			  snd_pcm_stream_t stream, int mode)
{
	...
	int card = -1, cchannels = CHANNELS, cchannels1 = CHANNELS;

	//SNDERR("_snd_pcm_softvol_open++, name:%s", name);
	...
	snd_config_for_each(i, next, conf) {
		...
		if (strcmp(id, "channels") ==0) {
			int chns;
			err = snd_config_get_integer(n, &chns);
			if (err < 0) {
				SNDERR("Invalid channels value");
				return err;
			}
			cchannels = chns;
			//SNDERR("softvol channels:%d", cchannels);
			continue;			
		}
	...
	//SNDERR("_snd_pcm_softvol_open, cchannels:%d, min_dB:%Lf, max_dB:%Lf, resolution:%d", cchannels, min_dB, max_dB, resolution);
	if (mode & SND_PCM_NO_SOFTVOL) {
		...
		if (err < 0)
			return err;
		if ((err = snd_pcm_parse_control_id(control, ctl_id, &card, &cchannels1, NULL)) < 0) {
			snd_pcm_close(spcm);
			return err;
		}
		//SNDERR("softvol channels1111:%d", cchannels);
